...
vector<double> findBin(simple_cpp_service::SimpleService::Request& req){
    ...
    //wrong input error handeling, if output is -1.0 then we recall the method again to repeat the process.
    set<int>size;
    size.insert(req.angle.size()), size.insert(req.clock.size()), size.insert(req.trash.size()), size.insert(req.distance.size());
    if(size.size()!=1){
    ...
    bool wall=false;
    bool bin=false;
    if(find(req.trash.begin(),req.trash.end(),2)!=req.trash.end())wall=true;
    if(find(req.trash.begin(),req.trash.end(),3)!=req.trash.end())bin=true;
    ...
    if(!bin && !wall){
        ...
        for(int i =0 ;i <req.distance.size();i++)maxDistance=std::max(maxDistance,req.distance[i]);
        
        auto i = find(req.distance.begin(),req.distance.end(),maxDistance);
        int index = std::distance(req.distance.begin(),i);
    ...
    if(bin){
        ...
        int binIndex = std::distance(req.trash.begin(),i);
    ...
    if(!bin && wall){
        ...
        for(int ind=0;ind<req.distance.size();ind++)if(req.trash[ind]==2)minDistanceWall=min(minDistanceWall,req.distance[ind]);
        auto i = find(req.distance.begin(),req.distance.end(),minDistanceWall);
        int wallIndex = std::distance(req.distance.begin(),i);
